...
* HookItem 的 entry() 内请不要用 try-catch 块包围整个方法来捕获未知异常, entry() 的调用处已有相关异常处理与日志打印
...
- 安装模块后第一次解析 DEX 时微信可能会崩溃: 临时解决方法: 重启重试; 解决方法: 禁用 '崩溃拦截 (Native)'

- 聊天工具栏必须先打开菜单至少一次才能正常工作: 暂未找到解决办法, 欢迎贡献修复
